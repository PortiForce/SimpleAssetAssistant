...
public sealed class AssetActivityBaseConfiguration : IEntityTypeConfiguration<AssetActivityBase>
{
	public void Configure(EntityTypeBuilder<AssetActivityBase> builder)
	{
		...
		_ = builder.Property(x => x.PlatformAccountId)
		...
		// Discriminator: Kind
		_ = builder.HasDiscriminator(x => x.Kind)
		...
		_ = builder.ComplexProperty(
			x => x.ExternalMetadata,
			meta =>
			{
				_ = meta.Property(m => m.ExternalId)
					.HasColumnName("ExternalId")
					.HasMaxLength(EntityConstraints.CommonSettings.ExternalIdMaxLength);

				_ = meta.Property(m => m.Fingerprint)
					.HasColumnName("Fingerprint")
					.HasMaxLength(EntityConstraints.CommonSettings.ExternalIdMaxLength);

				_ = meta.Property(m => m.Source)
					.HasColumnName("ExternalSource")
					.HasMaxLength(EntityConstraints.CommonSettings.ExternalIdMaxLength);

				_ = meta.Property(m => m.Notes)
		...
		_ = builder.HasIndex(x => new { x.PlatformAccountId, x.OccurredAt, x.Id })
