...
{
	public static ExternalIdentityId New() => new(GuidIdUtil.New());
	...
	public static ExternalIdentityId Parse(string raw)
	{
		...
		throw new FormatException(
			$"Value is not a valid non-empty {nameof(ExternalIdentityId)} GUID. Input (truncated): '{raw.Truncate()}'");
	}

	/// <summary>
	/// Attempts to parse the specified string representation of an external Identity identifier.
	/// </summary>
	/// <param name="raw">The string to parse as an external Identity identifier. Can be null.</param>
	/// <param name="id">When this method returns, contains the parsed <see cref="ExternalIdentityId"/> value if the parse operation succeeded, or
