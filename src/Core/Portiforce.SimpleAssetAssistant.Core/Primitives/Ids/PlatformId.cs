...
{
	public static PlatformId New() => new(GuidIdUtil.New());
	...
	public static PlatformId Parse(string raw)
	{
		...
		throw new FormatException(
			$"Value is not a valid non-empty {nameof(PlatformId)} GUID. Input (truncated): '{raw.Truncate()}'");
	}

	/// <summary>
	/// Attempts to parse the specified string representation of a platform identifier.
	/// </summary>
	/// <param name="raw">The string to parse as a platform identifier. Can be null.</param>
	/// <param name="id">When this method returns, contains the parsed <see cref="PlatformId"/> value if the parse operation succeeded, or
